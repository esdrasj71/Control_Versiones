...
Bill_Header.getNoFactura = result => {
    sql.query("SELECT MAX(Correlative_Number) + 1 as NoFactura FROM bill_header;", (err, res) => {
        if (err) {
            console.log("Error: ", err);
            result(err, null);
            result;
        }
        console.log("Numero de factura creada: ", res);
        result(null, res);
    })
}
...
Bill_Header.updateById = (id, bill_header, result) => {
    sql.query(
        "UPDATE bill_header SET Correlative_Number = ?, Serie = ?, Date = ?, Total = ?, Refund = ?, Annulment_State = ?, Payment_Complete = ?,Customers_Id = ?, Employee_Id = ? WHERE Bill_Header_Id = ?", [bill_header.Correlative_Number, bill_header.Serie, bill_header.Date, bill_header.Total, bill_header.Refund, bill_header.Annulment_State, bill_header.Payment_Complete, bill_header.Customers_Id, bill_header.Employee_Id, id],
        (err, res) => {
            if (err) {
                console.log("error: ", err);
                result(null, err);
