...
sql.query(`SELECT i.Inventory_Id, l.Product_Id,i.Stock, pp.Correlative_Product,pp.Name ,b.Name as Marca ,pc.Name as Categoria ,i.Unit_Price, i.Retail_Price, i.Wholesale_Price,l.Due_Date, l.Lot_Id  FROM inventory as i left join lot as l on i.Lot_Id=l.Lot_Id inner join product as pp on pp.Product_Id = l.Product_Id inner join brand as b on pp.Brand_Id = b.Brand_Id inner join product_category as pc on pp.Product_Category_Id = pc.Product_Category_Id where i.Inventory_Id= ${Inventario_Id}`, (err, res) => {
...
Inventory.updateById = (id, inventario, result) => {
  sql.query(
    "UPDATE inventory SET Stock = ?, Unit_Price = ?, Retail_Price = ?, Wholesale_Price = ?, Lot_Id = ?, Status = ? WHERE Inventory_Id = ?",
