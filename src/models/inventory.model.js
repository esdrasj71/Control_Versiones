...
Inventory.updateById = (id, inventario, result) => {
    sql.query(
        "UPDATE inventory SET Stock = ?, Unit_Price = ?, Retail_Price = ?, Wholesale_Price = ?, Lot_Id = ?, Status = ? WHERE Inventory_Id = ?", [inventario.Stock, inventario.Unit_Price, inventario.Retail_Price, inventario.Wholesale_Price, inventario.Lot_Id, inventario.Status, id],
        (err, res) => {
            if (err) {
                console.log("error: ", err);
                result(null, err);
