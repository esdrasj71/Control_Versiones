...
Inventory.updateById = (id, inventario, result) => {
    sql.query(
        "UPDATE inventory SET Stock = ?, Unit_Price = ?, Retail_Price = ?, Wholesale_Price = ?, Product_Id = ? WHERE Inventory_Id = ?", [inventario.Stock, inventario.Unit_Price, inventario.Retail_Price, inventario.Wholesale_Price, inventario.Product_Id, id],
        (err, res) => {
            if (err) {
                console.log("error: ", err);
                result(null, err);
